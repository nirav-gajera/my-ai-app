...
@section('title', 'Edit Telegram Bot')
...
@section('page-title', 'Edit Telegram Bot')
@section('page-subtitle', 'Update bot configuration and activation status.')
            ...
            <form method="POST" action="{{ route('admin.telegram-bots.update', $telegramBot) }}">
                ...
                <div class="form-group">
                    ...
                    <input id="name" name="name" type="text" class="form-input {{ $errors->has('name') ? 'input-error' : '' }}"
                           value="{{ old('name', $telegramBot->name) }}" >
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input id="token" name="token" type="text" class="form-input {{ $errors->has('token') ? 'input-error' : '' }}"
                           value="" placeholder="Leave blank to keep current token">
                    <p style="font-size: 12px; color: var(--text-muted); margin-top: 4px;">
                        Only fill this if you want to change the token. The current token is stored encrypted.
                    </p>
                    @error('token')
                        <p class="field-error">{{ $message }}</p>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input id="bot_url" name="bot_url" type="url" class="form-input {{ $errors->has('bot_url') ? 'input-error' : '' }}"
                           value="{{ old('bot_url', $telegramBot->bot_url) }}">
                    @error('bot_url')
                        <p class="field-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn btn-primary" id="submit-btn">Update Bot</button>
                </div>
            </form>
        </div>
    </div>

    {{-- Confirmation modal for replacing active bot --}}
    @if ($hasOtherActiveBot)
        <div class="confirm-modal" id="activate-confirm-modal" style="display:none">
            ...
            <div class="confirm-modal-box">
                ...
                <p class="confirm-modal-body">
                    Another Telegram bot is currently active. Activating this bot will <strong>deactivate the other one</strong>. Are you sure?
                </p>
...
<script>
    @if ($hasOtherActiveBot)
        ...
        const _wasActive = {{ $telegramBot->is_active ? 'true' : 'false' }};

        _form.addEventListener('submit', (e) => {
            if (_isActiveCheckbox.checked && !_wasActive && !_pendingSubmit) {
                e.preventDefault();
